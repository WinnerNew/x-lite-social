...
const SettingsView: React.FC<SettingsViewProps> = ({ onBack, onLogout }) => {
  const SettingItem = ({ icon: Icon, label, color = "text-zinc-100" }: any) => (
    <div className="flex items-center gap-4 px-4 py-4 cursor-pointer hover:bg-zinc-900/50 transition-colors border-b border-zinc-800/50 group">
    ...
    <div className="flex flex-col h-full bg-black">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="mt-8 border-t border-zinc-800">
          <button 
            onClick={onLogout}
            className="w-full flex items-center gap-4 px-4 py-4 text-pink-500 hover:bg-pink-500/5 transition-colors font-bold"
          >
            <LogOut size={20} />
            <span>Log out</span>
          </button>
